# 🚀 RFC-001: Reactless Event Delegation & Hydration Bridge
...
function StaticCard({ title, onAction }) {
  return (
    <div className="card">
      <h2>{title}</h2>
      {/* ⚠️ Each button creates Fiber nodes with event listeners */}
...
function StaticCard({ title, onAction }) {
  // 🚨 PROBLEM: How do we preserve React's event system?
  // 🚨 PROBLEM: How do we handle SSR hydration?
  // 🚨 PROBLEM: How do we maintain Context consumption?
...
```
┌─────────────────┐    ┌──────────────────┐    ┌─────────────────┐
│   React Tree    │    │  Reactless Bridge│    │  Static DOM     │
│                 │    │                  │    │                 │
│ • Fiber Nodes   │◄───│ • Event Proxy    │◄───│ • Native Nodes  │
│ • Context       │    │ • Context Bridge │    │ • Direct Updates│
│ • Refs          │    │ • Ref Forwarding │    │ • Template Cache│
└─────────────────┘    └──────────────────┘    └─────────────────┘
```
  ...
  attachContainer(container: HTMLElement, events: EventHandlers) {
    ...
    eventTypes.forEach((eventType) => {
      container.addEventListener(
        eventType,
        this.createDelegatedHandler(eventType, events),
      );
    });
  }
...
| ----------------------- | ------------- | ------------------------------ |
| **Events**              | ✅ Full       | All synthetic events supported |
| **Context**             | ✅ Partial    | Automatic boundary detection   |
| **Refs**                | ✅ Full       | Proxy system for DOM nodes     |
| **Error Boundaries**    | ✅ Full       | Propagation to React tree      |
| **Suspense**            | ✅ Full       | Boundary preservation          |
| **Concurrent Features** | ✅ Full       | No blocking behavior           |
